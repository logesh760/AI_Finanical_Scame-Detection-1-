...
import { AlertOctagon, AlertTriangle, ShieldCheck, Check, Trash2, ExternalLink } from 'lucide-react';
            ...
            alerts.map((alertItem) => (
              <div
                ...
                className={`bg-[#1A1A1A] border rounded-2xl p-5 space-y-4 shadow-lg transition-all hover:border-white/10 ${
                  alertItem.severity === 'High' ? 'border-red-500/10' : 'border-orange-500/10'
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex gap-3">
                    <div className={`p-2.5 rounded-xl shrink-0 ${
                      alertItem.severity === 'High' ? 'bg-red-500/10 text-red-500' : 'bg-orange-500/10 text-orange-400'
                    }`}>
                      {alertItem.severity === 'High' ? <AlertOctagon className="w-5 h-5 animate-pulse" /> : <AlertTriangle className="w-5 h-5" />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-sm font-bold text-white">{alertItem.title}</h3>
                        <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded border uppercase ${getSeverityStyles(alertItem.severity)}`}>
                ...
                <div className="flex gap-2 justify-end">
                  {alertItem.actions.map((act, index) => (
                    <button
                      key={index}
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-bold font-mono tracking-wide cursor-pointer transition-all border ${
                        act.toLowerCase().includes('block') || act.toLowerCase().includes('freeze')
